...
function TableHeader({ column, currentSort, onSort }: TableHeaderProps) {
...
function TableRowComponent({ row, columns, onClick }: TableRowProps) {
    ...
    switch (column.type) {
      case 'number':
        return typeof value === 'number'
          ? formatNumber(value, 'number', true)
          : value;
      case 'percentage':
      ...
      {columns.map((column, index) => (
        <td
          ...
          className={cn(
            'px-4 py-3 text-sm',
            index === 0 &&
              row.fillRatio !== undefined &&
              row.fillRatio > 0 &&
              'analytics-table-fill-cell'
          )}
          style={
            index === 0 && row.fillRatio !== undefined && row.fillRatio > 0
    ...
    <div className={cn('analytics-metric-card', className)}>
      <div className="space-y-4">
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className="grid grid-cols-4 gap-4 px-4 py-3 border-t border-border"
          >
